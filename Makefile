...
.PHONY: lattice
lattice:
	cargo run --bin reth -- lattice --dev ;
...
.PHONY: docker-build
docker-build:
	docker build . -t us-docker.pkg.dev/telcoin-network/telcoin-network/lattice ;

.PHONY: docker-push
docker-push:
	docker push us-docker.pkg.dev/telcoin-network/telcoin-network/lattice:latest ;
